...
The raster rows are split into `n_tile_rows` horizontal ranges with approximately the same number of valid pixels.
Inside each row range, raster columns are split into `n_tile_cols` vertical ranges with approximately the same number of valid pixels.
This generates:
```r
