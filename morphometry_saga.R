...
morphometry_saga <- function(dem,
  ...
  if (verbose == TRUE) {
    print("---------------------------- Início da geração de derivadas do MDE")
  } 
  ...
  if (sol_rad == TRUE) {
    ...
    if (verbose == TRUE) {
      print("Direct Insolation, Diffuse Insolation, Total Insolation, Direct to Diffuse Ratio, Duration of Insolation, Sunrise and Sunset Finished")
    }
  }
  ...
  rsaga.geoprocessor("ta_morphometry",
                     ...
                     list(DEM = dem,
                          AFH = paste0(outdir,
                                       "effective_air_flow_heights.sgrd"),
  ...
  rsaga.geoprocessor("ta_morphometry",
                     module = 7,
                     list(DEM = dem,
                          PROTECTION = paste0(outdir,
                                              "morphometric_protection_index.sgrd"),
  ...
  rsaga.geoprocessor("ta_morphometry",
                     module = 3,
                     list(ELEVATION = dem,
                          RESULT = paste0(outdir,
                                          "surface_specific_points.sgrd"),
                          METHOD = "1",
  ...
  rsaga.geoprocessor("ta_morphometry",
                     module = 22,
                     list(DEM = dem,
                          LANDFORMS = paste0(outdir,
                                             "terrain_surface_classification_iwahashi.sgrd"),
                          CONV_RECALC = 0,
                          TEXTURE = 0,
                          TEXT_RECALC = 0,
                          TYPE = 2,
  ...
  rsaga.geoprocessor("ta_morphometry",
                     module = 21,
                     list(DEM = dem,
                          CONVEXITY = paste0(outdir,
                                             "terrain_surface_convexity.sgrd"),
                          KERNEL = 0,
                          TYPE = 0,
                          EPSILON = 0,
                          SCALE = 3,
  ...
  rsaga.geoprocessor("ta_morphometry",
                     module = 20,
                     list(DEM = dem,
                          TEXTURE = paste0(outdir,
                                           "terrain_surface_texture.sgrd"),
                          EPSILON = 1,
                          SCALE = 10,
                          METHOD = 1,
                          DW_WEIGHTING = 3,
  ...
  rsaga.geoprocessor("ta_morphometry",
                     ...
                     list(DEM = dem,
                          TPI = paste0(outdir,
                                       "topographic_position_index.sgrd"),
                          STANDARD = 0,
                          RADIUS_MIN = 0,
                          RADIUS_MAX = 100,
                          DW_WEIGHTING = 0,
  ...
  rsaga.wetness.index(in.dem = dem,
                      out.wetness.index = paste0(outdir,
                                                 "saga_wetness_index.sgrd"),
                      flags = "s", env = env)
  ...
  if (verbose == TRUE) {
    print("-------------------------------- Fim da geração de derivadas do MDE")
    
    
    print("------------------------- Inicio da conversão dos arquivos para tif")
  }
  ...
  if (verbose == TRUE) {
    if (align_rasters == TRUE) {
      print("---------------------------------- Fim do alinhamento dos rasters")
    }
  }
  
  if (verbose == TRUE) {
    print("---------------------------- Fim da conversão dos arquivos para tif")
    print("----------------------------------------- Apagando arquivos não tif")
  }
  ...
  if (verbose == TRUE) {
    print("----------------------------------------- Arquivos não tif apagados")
  }
