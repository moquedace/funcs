pred_writer_raster_prob_raw_resample <- function(model = NULL,
                                        varfact = NULL,
                                        tile = NULL,
                                        custom_crs = NULL,
                                        outdir = "./predict",
  ...
  for (t in type_predict) {
    if (t == "prob") {
      ...
      r_res <- predict_by_chunks(model, newdata, type = t,
                                 n_divisions = n_divisions)
      gc()
      for (class_name in colnames(r_res)) {
        class_raster <- rast(cbind(xy, r_res[, class_name, drop = FALSE]),
                             type = "xyz", crs = crs_str) %>% 
          resample(y = rast(r_resample), method = "near", threads = TRUE)
  ...
  for (t in names(results)) {
    ...
    writeRaster(results[[t]], filename = file_path, gdal = "COMPRESS=LZW",
                overwrite = TRUE)
