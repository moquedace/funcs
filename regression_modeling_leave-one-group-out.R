...
make_leave_one_group_out_index <- function(group) {
  ...
  index_list <- lapply(groups, function(current_group) {
      ...
      for (n in start_run:n_units) {
        ...
        if (run_rfe && length(rfe_candidate_predictors) > 0) {
          ...
          rfe_ctrl <- rfeControl(
            ...
            method = inner_method,
            index = if (kfold) inner_index else NULL,
            verbose = FALSE,
            allowParallel = TRUE
          )
          ...
          rfe_result <- if (!is.null(rfe_fit$results)) {
            rfe_fit$results
          } else {
            rfe_fit$result
          }
          
          lrferes[[n]] <- rfe_result
