tile_raster_path <- function(rst, rows, cols, outdir, graph = TRUE,
  ...
  if (!inherits(rst_rast, "SpatRaster")) stop("O parâmetro 'rst' deve ser um objeto 'SpatRaster'.")
  ...
  crs_str <- if (is.null(custom_crs)) {
    crs_info <- crs(rst_rast, describe = TRUE)
  ...
  r_tile <- rast(ncols = cols, nrows = rows, nlyrs = nlyr(rst_rast), crs = crs_str, extent = ext(rst_rast))
  
  # Extensão dos tiles
  message("Calculando as extensões dos tiles...")
  blocks <- terra::getTileExtents(rst_rast, r_tile, extend = extend) %>%
    apply(1, ext) %>%
    lapply(vect) %>%
    lapply(`crs<-`, crs_str)
