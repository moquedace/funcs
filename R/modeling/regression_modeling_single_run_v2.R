...
dfbase <- safe_read_csv(
  "./extract_xy/xy_mixed.csv"   # <-- EDIT HERE: caminho do dataset
) %>%
  dplyr::select(
    -c(1:2)   # <-- EDIT HERE: remove as 2 primeiras colunas (id/coords) por POSICAO
  )
...
# <-- EDIT HERE: variaveis-alvo. Defina por NOME para nao depender da ordem das colunas.
# Ex.: target_vars <- c("clay", "sand", "silt")
target_vars <- NULL  # NULL => comportamento antigo names(dfbase)[18:1], porem com aviso

if (is.null(target_vars)) {
  warning("target_vars = NULL: usando selecao por POSICAO names(dfbase)[18:1]. Defina target_vars por NOME para um pipeline robusto.")
  varsy <- names(dfbase)[18:1]
} else {
  missing_targets <- setdiff(target_vars, names(dfbase))
  if (length(missing_targets) > 0) {
    stop(paste0("target_vars nao encontrados em dfbase: ", paste(missing_targets, collapse = ", ")))
  }
  varsy <- target_vars
}

cat("Targets a modelar: ", paste(varsy, collapse = ", "), "\n")
...
for (i in seq_along(models)) {
  ...
  for (j in seq_along(varsy)) {   # <-- v2: modela todos os alvos de varsy (removido o subset fixo [3:18])
    ...
    if (kfold) {
      ...
      rfe_index <- validate_resampling_index(
        index = rfe_index,
        n_rows = nrow(dyx_sel)
      )
      ...
      model_index <- validate_resampling_index(
        index = model_index,
        n_rows = nrow(dyx_sel)
      )
